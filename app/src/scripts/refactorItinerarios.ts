...
function calcularSimilaridade(str1: string, str2: string): number {
  ...
  for (const p1 of palavras1) {
    for (const p2 of palavras2) {
      if (p1 === p2 && p1.length > 2) { // Ignora palavras muito curtas
        palavrasEmComum++;
        break;
      }
    }
...
function encontrarParadaPorNome(
  ...
  let melhorMatch: { id: string; similaridade: number; nomeEncontrado: string } | null = null;
...
export function refatorarItinerario(
  ...
  for (let i = 0; i < itinerarioParadasIds.length; i++) {
    ...
    if (mapeamentoManual[nomeNormalizado]) {
      ...
      console.log(`✓ [${linhaId}] Parada ${i + 1}: "${nomeParada}" -> ${mapeamentoManual[nomeNormalizado]} (mapeamento manual)`);
      continue;
    }
    ...
    if (resultado && resultado.similaridade >= 0.8) {
      ...
      console.log(`✓ [${linhaId}] Parada ${i + 1}: "${nomeParada}" -> ${resultado.id} (${resultado.nomeEncontrado}) [similaridade: ${(resultado.similaridade * 100).toFixed(1)}%]`);
    } else if (resultado && resultado.similaridade >= 0.6) {
      novoItinerario.push(resultado.id);
      console.warn(`⚠ [${linhaId}] Parada ${i + 1}: "${nomeParada}" -> ${resultado.id} (${resultado.nomeEncontrado}) [similaridade: ${(resultado.similaridade * 100).toFixed(1)}%] - VERIFICAR MANUALMENTE`);
      avisos.push(`Parada ${i + 1}: "${nomeParada}" mapeada para ${resultado.id} com similaridade baixa (${(resultado.similaridade * 100).toFixed(1)}%)`);
    } else {
      // Mantém o nome original se não encontrar correspondência
      novoItinerario.push(nomeParada);
      console.error(`✗ [${linhaId}] Parada ${i + 1}: Não foi possível encontrar correspondência para "${nomeParada}"`);
...
const mapeamentoAdicional: Record<string, string> = {
  ...
  'UNIDADE ADMINISTRATIVA III (DAP/DRCA/DRH/COPEVE/PROJETO MANUELZÃO/EDUCAÇÃO A DISTÂNCIA/ESTUDOS TRANSCIPLINARES/ESTUDOS DE CRIMINALIDADE E SEGURANÇA/ESTUDOS SOBRE ENSINO SUPERIOR E POLÍTICAS PÚBLICAS)': 'P06',
