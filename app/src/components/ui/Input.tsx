...
export const inputVariants = tv({
  ...
  variants: {
    ...
    error: {
      true: 'border-danger-border focus:border-danger-border focus:ring-danger-border',
      false: '',
    },
  },
  defaultVariants: {
      ...
      <div data-slot="search-input" className="relative w-full">
        ...
        {showClear && hasValue ? (
          ...
          <div className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 hidden md:block">
            <kbd className="inline-flex h-5 items-center rounded border border-card-border bg-background px-1.5 text-micro font-medium text-text-tertiary font-mono">
              {shortcut}
            </kbd>
          </div>
        ) : null}
