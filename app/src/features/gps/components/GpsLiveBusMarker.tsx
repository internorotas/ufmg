...
function criarIcone(corHex: string, heading: number | null, isLive: boolean): L.DivIcon {
  ...
  const bg = isLive ? corHex : hexToRgba(corHex, 0.6);
      ...
      <div style="position:relative;width:36px;height:36px;">
        ${mostrarSeta ? `
          <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;transform:rotate(${rotacao}deg);">
            <div style="width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-bottom:12px solid ${hexToRgba(corHex, 0.7)};transform:translateY(-13px);"></div>
          </div>` : ''}
...
export function GpsLiveBusMarker({ linha, todasParadas }: GpsLiveBusMarkerProps) {
  ...
  return (
    <Marker position={[pos.lat, pos.lng]} icon={icon}>
      <Popup>
        <div style={{ minWidth: '172px', fontFamily: 'sans-serif', fontSize: '13px' }}>
          {/* Cabeçalho: badge + nome + AO VIVO */}
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            ...
            <span style={{ fontWeight: '600', color: '#111', flex: 1, minWidth: 0 }}>
              {linha.nome}
            </span>
            {isLive && (
              <span
          ...
          {/* Corpo */}
          <div style={{ color: '#6b7280', lineHeight: '1.65', fontSize: '12px' }}>
            {isLive ? (
              <>
                <div>📡 Posição compartilhada por usuário</div>
                <div>
                  Confiança:{' '}
              ...
              <>
                ...
                <div>
                  Saída às{' '}
                  <strong style={{ color: '#111' }}>{theoreticalPos!.horarioSaida}</strong>
                  {' · '}há {Math.round(theoreticalPos!.elapsedMin)} min
                </div>
              </>
            )}
          </div>
