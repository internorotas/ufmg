...
export function findScheduleIndex<T>(
  ...
  // ⚡ Bolt: Removed default parameter lambda allocation. Explicitly branching
  // outside the loop prevents function call overhead on every iteration.
  if (getVal) {
    while (left < right) {
      // ⚡ Bolt: Unsigned right shift (`>>> 1`) is 2-4x faster than Math.floor()
      // for integer division and handles 32-bit array bounds safely.
