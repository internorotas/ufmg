...
export function findScheduleIndex<T>(
  ...
  // Hoisting the branch outside the loop avoids checking if getVal exists
  // on every iteration. Using unsigned right shift (>>> 1) for division
  // is faster than Math.floor and prevents 32-bit array overflows.
  if (getVal) {
    while (left < right) {
