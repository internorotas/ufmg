...
export function findScheduleIndex<T>(
  ...
  if (getVal) {
    while (left < right) {
      // Unsigned right shift (>>> 1) is faster than Math.floor((left + right) / 2)
      // and prevents overflow for very large arrays.
    ...
    // Fast path: avoid function call overhead when no accessor is provided.
    while (left < right) {
      const mid = (left + right) >>> 1;
