...
/**
 * Retorna os horários da linha para o dia atual parseados e ordenados em minutos.
 * Utiliza WeakMap para evitar O(N log N) e O(N) calls em recálculos redundantes
...
export function obterHorariosMinutosLinhaNoDia(linha: Linha, dataAtual: Date): number[] {
  ...
  const cached = _horariosMinutosCache.get(horariosDia);
  if (cached) {
    return cached;
  }
  ...
  _horariosMinutosCache.set(horariosDia, parsed);
  return parsed;
}
