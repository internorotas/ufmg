...
export function calcularPrevisaoChegada(
  ...
  const horariosMinutos = obterHorariosMinutosLinhaNoDia(linha, agoraSaoPaulo);
  if (horariosMinutos.length === 0) return null;
  ...
  for (let i = 0; i < horariosMinutos.length; i++) {
    const saidaMinutos = horariosMinutos[i];
