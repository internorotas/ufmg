...
import { isCacheableApiRequest } from './pwa/apiCachePolicy';
...
const API_CACHE_NAME = getTenantCacheName('api-v2');
...
registerRoute(
  ({ url, request }: { url: URL; request: Request }) => isCacheableApiRequest(url, request),
  new NetworkFirst({
